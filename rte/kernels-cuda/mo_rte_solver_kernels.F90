...
module mo_rte_solver_kernels
  ...
  interface
    ...
    subroutine lw_solver_noscat_gaussquad_wrapper_(ncol, nlay, ngpt, top_at_1, nmus, Ds,              & 
                                                  weights, tau, lay_source, lev_source_inc, lev_source_dec, &
                                                  sfc_emis, sfc_src, flux_up, flux_dn, sfc_srcJac, flux_upJac) bind(C)
      use iso_c_binding, only: c_int, c_double, c_bool
      use mo_rte_kind,       only: wp, wl

      implicit none
      integer(kind=c_int),                   intent(in   ) :: ncol, nlay, ngpt ! Number of columns, layers, g-points
      logical(kind=wl),                      intent(in   ) :: top_at_1
      integer(kind=c_int),                   intent(in   ) :: nmus         ! number of quadrature angles
...
contains
  ...
  subroutine lw_solver_noscat_GaussQuad(ncol, nlay, ngpt, top_at_1, &
    ...
    real(wp), dimension(nmus)                            :: ds1D

    ds1D(:) = Ds(1, 1, :)

    !$acc data copyout(broadband_up, broadband_dn) if (do_broadband)
    !$acc data create(gpt_Jac, ds1D)
    !$acc data copyin(ds1D, weights)
    !$acc host_data use_device(ds1D, weights, tau, lay_source, lev_source_inc, lev_source_dec, sfc_emis, sfc_src, flux_up, flux_dn, sfc_srcJac, gpt_Jac)
    call lw_solver_noscat_gaussquad_wrapper(ncol, nlay, ngpt, top_at_1, nmus, ds1D,                   & 
                                            weights, tau, lay_source, lev_source_inc, lev_source_dec, &
                                            sfc_emis, sfc_src, flux_up, flux_dn, sfc_srcJac, gpt_Jac)
    !$acc end host_data
    !$acc end data
    ...
    if(do_broadband) then
      ...
      !   conversion from intensity to flux assuming azimuthal isotropy and quadrature weight
      !
      call sum_broadband_factor(ncol, nlay+1, ngpt, 1._wp, flux_dn, broadband_dn)
      call sum_broadband_factor(ncol, nlay+1, ngpt, 1._wp, flux_up, broadband_up)
    end if
    !$acc        exit data copyout( flux_dn,flux_up)
    !$omp target exit data map(from:flux_dn,flux_up)
    !
    ...
    !
    if (do_Jacobians) then
      call sum_broadband_factor(ncol, nlay+1, ngpt, 1._wp, gpt_Jac, flux_upJac)
    end if
    !$acc end data
    !$acc end data
